...
- **CLI client** — a built-in `oris-memory-server` binary that speaks the MCP JSON-RPC protocol over stdio.
...
|-------|------|---------------|
| `oris-control-plane` | HTTP + MCP server, control plane, key/PKI services, client | `oris-memory-store`, `oris-memory-contract` |
| `oris-memory-contract` | Canonical cross-agent experience types (`GeneV1`, `CapsuleV1`, `ExperienceBundleV1`, `UsageReceiptV1`) | none |
| `oris-memory-store` | SQLite gene/capsule store, migrations, replay hooks | none |

```
oris-control-plane
├── oris-memory-store            (0 oris deps)
└── oris-memory-contract  (0 oris deps)
```

The dependency graph is acyclic and closed: the two leaf crates have no path dependencies, and `oris-control-plane` references them only via relative workspace paths. This makes the set trivially extractable as a standalone product.
