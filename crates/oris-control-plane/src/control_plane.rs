...
impl ExperienceControlPlane {
    ...
    pub fn bundle(
        ...
        Ok(ExperienceBundleV1 {
            schema_version: oris_memory_contract::EXPERIENCE_BUNDLE_V1.into(),
            gene,
            capsules,
            usage_receipts,
        })
...
fn structurally_compatible(gene: &GeneV1, query: &ExperienceSearchQuery) -> bool {
    ...
    // Always enforce tenant isolation: a gene with an empty `tenant_ids`
    // list is shared across all tenants; otherwise the requesting tenant
    // must appear in the list.
    if !gene.applicability.tenant_ids.is_empty()
        && !gene.applicability.tenant_ids.contains(&query.tenant_id)
    {
        return false;
    }
    ...
    let matched = gene
        ...
        .filter(|constraint| {
            ...
            match constraint.operator {
                oris_memory_contract::ConstraintOperator::Equals => actual == &constraint.value,
                oris_memory_contract::ConstraintOperator::NotEquals => actual != &constraint.value,
                oris_memory_contract::ConstraintOperator::Contains => actual
                    .to_string()
                    .contains(constraint.value.as_str().unwrap_or("")),
                oris_memory_contract::ConstraintOperator::Exists => true,
                oris_memory_contract::ConstraintOperator::Semver => actual
                    .as_str()
...
mod tests {
    use super::*;
    use oris_memory_contract::AdoptionStatus;
    ...
    fn search_normalizes_cross_agent_tool_capabilities() {
        ...
        bundle.gene.tool_requirements = vec![
            oris_memory_contract::ToolRequirement {
    ...
    fn search_enforces_tenant_isolation() {
        let store = ExperienceControlPlane::memory().unwrap();
        let mut bundle = fixture();
        bundle.gene.applicability.tenant_ids = vec!["tenant-a".into()];
        store.propose(&bundle).unwrap();
        let base = || ExperienceSearchQuery {
            text: "transient rust http timeout".into(),
            available_tools: vec!["cargo".into()],
            environment: BTreeMap::from([("language".into(), serde_json::json!("rust"))]),
            ..Default::default()
        };
        // Same tenant — gene is visible.
        let own = ExperienceSearchQuery {
            tenant_id: "tenant-a".into(),
            ..base()
        };
        assert_eq!(store.search(&own).unwrap().len(), 1);
        // Different tenant — gene is hidden.
        let cross = ExperienceSearchQuery {
            tenant_id: "tenant-b".into(),
            ..base()
        };
        assert!(store.search(&cross).unwrap().is_empty());
    }
}
