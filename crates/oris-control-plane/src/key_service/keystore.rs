...
impl KeyStore {
    ...
    /// Load capability scopes for a key from the `api_key_scopes` table.
    pub fn load_scopes(&self, key_id: &str) -> Result<Vec<String>, KeyServiceError> {
        let mut stmt = self
            .conn
    ...
    fn with_scopes(&self, mut info: ApiKeyInfo) -> Result<ApiKeyInfo, KeyServiceError> {
        info.scopes = self.load_scopes(&info.key_id.0)?;
